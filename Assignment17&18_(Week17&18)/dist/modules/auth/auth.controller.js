...
router.post("/login", (0, middleware_1.validation)(validators.login), async (req, res, next) => {
    const data = await auth_service_1.default.login(req.body, req.host);
...
router.post("/request-forget-password-code", (0, middleware_1.validation)(validators.requestForgotPasswordCode), async (req, res, next) => {
    await auth_service_1.default.requestForgotPasswordCode(req.body.email);
    return (0, response_1.successResponse)({ res, status: 201 });
});
router.patch("/verify-forget-password-code", (0, middleware_1.validation)(validators.verifyForgotPasswordCode), async (req, res, next) => {
    await auth_service_1.default.verifyForgotPasswordCode(req.body.email, req.body.otp);
    return (0, response_1.successResponse)({ res, status: 200 });
});
router.patch("/reset-forget-password-code", (0, middleware_1.validation)(validators.resetForgotPasswordCode), async (req, res, next) => {
    await auth_service_1.default.resetForgotPasswordCode(req.body);
    return (0, response_1.successResponse)({ res, status: 200 });
});
